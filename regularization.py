...
class Perturbation:
    ...
    @classmethod
    def add_knn_noise_to_tensor(cls, tens: torch.Tensor, k) -> torch.Tensor:
        """
        Adds noise to a tensor based on K-nearest neighbor distances using GPU operations.
        :param tens: input tensor
        :param k: number of nearest neighbors to consider
        :return: noisy tensor in the same shape as input
        """
        # Compute pairwise distances, tens should already be on the GPU
        pairwise_distances = torch.cdist(tens, tens)

        # Set diagonal to a very high value to exclude self-distance
        pairwise_distances.fill_diagonal_(1e10)

        # Obtain the distances of the k-nearest neighbors
        distances, _ = torch.topk(pairwise_distances, k=k, largest=False, dim=1)

        # Calculate the mean distances
        mean_distances = distances.mean(dim=1)

        # Create noise based on mean distances
        noise = torch.randn_like(tens) * mean_distances.unsqueeze(1)

        return tens + noise

    @classmethod
    def perturb_tensor(cls, tens: torch.Tensor, n_samples: int, k, perturbation: bool = True) -> torch.Tensor:
        """
        ...
        :param k: number of nearest neighbors to consider for noise
        ...
        if perturbation:
            ...
            tens = cls.add_knn_noise_to_tensor(tens, k)
...
class RegParameters(object):
    ...
    """
    def __init__(self, lambda_image: float = 1e-10, lambda_color: float = 1e-10, norm: float = 2.0, estimation: str = 'ent',
                 optim_method: str = 'max_ent', n_samples: int = 10, grad: bool = True, knn_value: int = 219):
